    ...
    for kata = 1:60
        ...
        if statAll == 1
            ...
            if length(signal_1D) > 24
                ...
                subplot(5,1,1); plot(time_axis, signal_delta); title('Delta (0.5-4 Hz)'); ylabel('Amplitudo (\muV)'); grid on;
                subplot(5,1,2); plot(time_axis, signal_theta); title('Theta (4-8 Hz)'); ylabel('Amplitudo (\muV)'); grid on;
                subplot(5,1,3); plot(time_axis, signal_alpha); title('Alpha (8-13 Hz)'); ylabel('Amplitudo (\muV)'); grid on;
                subplot(5,1,4); plot(time_axis, signal_beta); title('Beta (13-30 Hz)'); ylabel('Amplitudo (\muV)'); grid on;
                subplot(5,1,5); plot(time_axis, signal_gamma); title('Gamma (30-49 Hz)'); ylabel('Amplitudo (\muV)'); xlabel('Waktu (detik)'); grid on;
                
                sgtitle(strcat('Analisis Gelombang EEG - Sesi: ', num2str(session), ', Kata: ', num2str(kata)));
